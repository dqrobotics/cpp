...
TranslationFirstPoseController::TranslationFirstPoseController(DQ_kinematics robot, MatrixXd translation_feedback_gain, MatrixXd rotation_feedback_gain, double translation_damping, double rotation_damping) : DQ_controller()
{
    ...
    identity_           = Matrix<double,4,4>::Identity();

    error_translation_  = MatrixXd(4,1);
	error_rotation_     = MatrixXd(4,1);

    end_effector_pose_ = DQ(0,0,0,0,0,0,0,0);
	translation_       = DQ(0,0,0,0,0,0,0,0);
	rotation_          = DQ(0,0,0,0,0,0,0,0);

}

VectorXd TranslationFirstPoseController::getNewJointPositions(DQ reference, VectorXd thetas)
{
...
VectorXd TranslationFirstPoseController::getNewJointVelocities(DQ reference, VectorXd thetas)
{
	...
	//Calculate Jacobians
	translation_jacobian_ = translationJacobian(robot_, analytical_jacobian_, end_effector_pose_);
	...
	translation_jacobian_pseudoinverse_ = (translation_jacobian_.transpose())
										 *(translation_jacobian_*translation_jacobian_.transpose() + translation_damping_*translation_damping_*identity_).inverse();


	rotation_jacobian_pseudoinverse_ =    (rotation_jacobian_.transpose())
										 *(rotation_jacobian_*rotation_jacobian_.transpose() + rotation_damping_*rotation_damping_*identity_).inverse();

...
std::tuple<DQ, DQ> DQ_Geometry::closest_points_between_lines(const DQ &line1, const DQ &line2)
{
    ...
    const DQ& p1 = (cross(-1.0*m1,cross(l2,cross(l1,l2))) + dot(m2,cross(l1,l2) )*l1 )*pow( 1./vec4(cross(l1,l2)).norm(),2);
    const DQ& p2 = (cross( m2,cross(l1,cross(l1,l2))) - dot(m1,cross(l1,l2) )*l2 )*pow( 1./vec4(cross(l1,l2)).norm(),2);
...
{
    if(!is_line(line_1))
        throw std::runtime_error("DQ_Geometry::line_segment_to_line_squared_distance::Input line_1 must be a line.");
    if(!is_line(line_2))
        throw std::runtime_error("DQ_Geometry::line_segment_to_line_squared_distance::Input line_2 must be a line.");
    if(!is_pure_quaternion(line_1_point_1))
        throw std::runtime_error("DQ_Geometry::line_segment_to_line_squared_distance::Input line_1_point_1 must be a point.");
    if(!is_pure_quaternion(line_1_point_2))
        throw std::runtime_error("DQ_Geometry::line_segment_to_line_squared_distance::Input line_1_point_2 must be a point.");
    if(!is_pure_quaternion(line_2_point_1))
        throw std::runtime_error("DQ_Geometry::line_segment_to_line_squared_distance::Input line_2_point_1 must be a point.");
    if(!is_pure_quaternion(line_2_point_2))
        throw std::runtime_error("DQ_Geometry::line_segment_to_line_squared_distance::Input line_2_point_2 must be a point.");

    const DQ& m1 = D(line_1);
    const DQ& l1 = P(line_1);

    if(cross(line_1_point_1,l1) != m1)
        throw std::runtime_error("DQ_Geometry::line_segment_to_line_squared_distance::Input line_1_point_1 must be on line_1.");
    if(cross(line_1_point_2,l1) != m1)
        throw std::runtime_error("DQ_Geometry::line_segment_to_line_squared_distance::Input line_1_point_2 must be on line_1.");

    const DQ& m2 = D(line_2);
    const DQ& l2 = P(line_2);

    if(cross(line_2_point_1,l2) != m2)
        throw std::runtime_error("DQ_Geometry::line_segment_to_line_squared_distance::Input line_2_point_1 must be on line_2.");
    if(cross(line_2_point_2,l2) != m2)
        throw std::runtime_error("DQ_Geometry::line_segment_to_line_squared_distance::Input line_2_point_2 must be on line_2.");

...
DQ_PseudoinverseController::DQ_PseudoinverseController(DQ_Kinematics *robot):
    DQ_KinematicController (robot)
{

}

DQ_PseudoinverseController::DQ_PseudoinverseController(const std::shared_ptr<DQ_Kinematics> &robot):
    DQ_KinematicController(robot)
{
...
{
    return DQ_PseudoinverseController::compute_tracking_control_signal(q,task_reference,VectorXd::Zero(task_reference.size()));
}

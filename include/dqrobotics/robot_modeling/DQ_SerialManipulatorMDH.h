...
/**
 * @brief Concrete serial manipulator based on the modified Denavit-Hartenberg convention.
 *
 ...
 *
 * @see DQ_SerialManipulator, DQ_SerialManipulatorDH, DQ_ParameterDH
 */
...
protected:
     ...
     * @return The dual quaternion term associated with the ith joint under the modified DH convention.
     */
    DQ _get_w(const int& ith) const;
    /**
     * @brief Converts the ith modified DH link transform into a dual quaternion.
     *
...
public:
    /**
     * @brief Returns one row of the stored modified DH matrix.
     *
     * @param parameter_type Modified DH parameter to be retrieved.
    ...
    /**
     * @brief Returns a modified DH parameter of a specific joint.
     *
     * @param parameter_type Modified DH parameter to be retrieved.
    ...
    /**
     * @brief Replaces one row of the stored modified DH matrix.
     *
     * @param parameter_type Modified DH parameter to be updated.
    ...
    /**
     * @brief Sets a modified DH parameter of a specific joint.
     *
     * @param parameter_type Modified DH parameter to be updated.
     * @param to_ith_link Joint index.
    ...
    /**
     * @brief Returns the joint types supported by the modified DH implementation.
     *
    ...
    /**
     * @brief Returns the theta row of the stored modified DH matrix.
    ...
    /**
     * @brief Returns the d row of the stored modified DH matrix.
    ...
    /**
     * @brief Returns the a row of the stored modified DH matrix.
    ...
    /**
     * @brief Returns the alpha row of the stored modified DH matrix.
    ...
    /**
     * @brief Returns the joint-type row of the stored modified DH matrix.
    ...
    /**
     * @brief Constructs a serial manipulator from a modified DH matrix.
     *
     * @param mdh_matrix 5 x n matrix containing theta, d, a, alpha, and joint-type rows.
     * @throws std::range_error If @p mdh_matrix does not have exactly 5 rows.
     */
    ...
    /**
     * @brief Computes the raw pose Jacobian under the modified DH convention.
     *
    ...
    /**
     * @brief Computes the time derivative of the raw pose Jacobian under the modified DH convention.
     *
    ...
    /**
     * @brief Computes the raw forward kinematics under the modified DH convention.
     *
     * @param q_vec Vector containing the robot joint configurations.
     * @param to_ith_link Index of the last link to be accounted for in the computation.
